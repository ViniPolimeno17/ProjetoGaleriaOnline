...
export const Galeria = () => {
  ...
  async function cadastrarCard(e) {
    ...
    if (imagem && nomeImagem) {
      try {
        //FormData é uma interface JavaScript que permite contruir um conjunto de pares
        //chave/valor representado os dados de um formulario HTML.
        ...
        {cards.length > 0 ? (
          cards.map((e) => (
            <Card
              key={e.id}
              tituloCard={e.nome}
              imgCard={`https://localhost:7032/${e.caminho.replace("wwwroot/", "")}`}
